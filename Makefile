...
TESTS = test-interface-st-hbw test-interface-mt-hbw test-mt-hbw test-st-hbw
...
test-st-hbw: test-single-thread.c 
	$(HERMIT-CC) -DSIZE=16384 -DNUM_ITER=10 $< $(LIBHBWMALLOC) -o $@ 

test-st-native: test-single-thread.c 
	$(HERMIT-CC) -DSIZE=16384 -DNUM_ITER=10 -DNATIVE $< -o $@ 

test-mt-hbw: test-multi-thread.c
	$(HERMIT-CC) $(THREAD_FLAG) -DSIZE=16384 -DNUM_ITER=10 -DNUM_THREADS=3 $< $(LIBHBWMALLOC) -o $@ 

test-mt-native: test-multi-thread.c
	$(HERMIT-CC) $(THREAD_FLAG) -DSIZE=16384 -DNUM_ITER=10 -DNUM_THREADS=3 -DNATIVE $< -o $@ 

test-mt-linux: test-multi-thread.c
	$(CC) $(THREAD_FLAG) -DSIZE=16384 -DNUM_ITER=10 -DNUM_THREADS=3 -DNATIVE -DLINUX $< -o $@ 
